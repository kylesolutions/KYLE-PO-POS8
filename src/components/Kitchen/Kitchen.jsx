...
function Kitchen() {
  ...
  const fetchKitchenNotes = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch Kitchen Notes: ${response.status} - ${response.statusText}`
        );
      }
      ...
      if (data.message?.status !== "success") {
        throw new Error(
          data.message?.message || data.exception || "Invalid response from server"
        );
      }
      ...
      const storedPreparedOrders =
        JSON.parse(localStorage.getItem("preparedOrders")) || {};
      ...
      const storedPreparedItems =
        JSON.parse(localStorage.getItem("preparedItems")) || [];
      ...
      setError(
        `Failed to load kitchen notes: ${errorMessage}. Please try again or contact support.`
      );
      setKitchenNotes([]);
      setAllKitchens([]);
    } finally {
  ...
  const getFilteredItems = (kitchen) => {
    ...
    kitchenNotes.forEach((order) => {
      order.cartItems.forEach((item) => {
        if (
          (kitchen === "All" || item.kitchen === kitchen) &&
        ...
        setKitchenNotes((prev) =>
          prev.map((order) => ({
            ...
            cartItems: order.cartItems.filter(
              (cartItem) =>
                cartItem.id !== id ||
                (cartItem.status !== "Prepared" && cartItem.status !== "Dispatched")
            ),
          }))
        );
  ...
  const handleSelectAll = (kitchen) => {
    ...
    const numSelectedInTable = tableItems.filter((item) =>
      selectedItems.includes(item.id)
    ).length;

    if (numSelectedInTable === tableItems.length) {
      setSelectedItems((prev) =>
        prev.filter((id) => !tableItemsIds.includes(id))
      );
    } else {
      setSelectedItems((prev) => [
        ...new Set([...prev, ...tableItemsIds.filter((id) => !prev.includes(id))]),
      ]);
    }
  };
  ...
  const renderKitchenSection = (kitchen) => {
    ...
    const numSelectedInTable = items.filter((item) =>
      selectedItems.includes(item.id)
    ).length;
        ...
        <>
          <div className="controls-section">
            <div className="kitchen-tabs">
              <button
                className={`kitchen-tab ${selectedKitchen === "All" ? "active" : ""}`}
              ...
              {allKitchens.map((kitchen) => (
                <button
                  key={kitchen}
                  className={`kitchen-tab ${
                    selectedKitchen === kitchen ? "active" : ""
                  }`}
