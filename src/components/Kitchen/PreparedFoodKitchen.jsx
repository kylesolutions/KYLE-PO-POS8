...
function PreparedFoodKitchen() {
    const { savedOrders, updateOrderStatus, informBearer, preparedItems } = useContext(UserContext);
    const navigate = useNavigate();

    const preparedFoodKitchenOrders = savedOrders.filter(order =>
        order.cartItems.some(item => item.kitchenType === "Prepared Food Kitchen")
    );

    const groupedOrders = preparedFoodKitchenOrders.reduce((groups, order) => {
    ...
    const handleInformBearer = () => {
        const preparedOrders = preparedFoodKitchenOrders.map(order => ({
            ...order,
  ...
  return (
    ...
    <h3 className="text-center">Prepared Food Kitchen</h3>
    {Object.keys(groupedOrders).length === 0 ? (
        <p>No orders for Prepared Food Kitchen.</p>
    ) : (
                ...
                <tbody>
                    {Object.entries(groupedOrders).map(([customer, orders]) =>
                        orders.map((order, orderIndex) =>
                            order.cartItems
                                .filter(item => item.kitchenType === "Prepared Food Kitchen")
                                .map((item, itemIndex) => (
...
export default PreparedFoodKitchen